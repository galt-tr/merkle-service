// Package cache provides a high-performance caching layer.
// This file implements the ImprovedCache - a memory-efficient, highly concurrent cache
// optimized for storing key-value pairs with minimal GC pressure.
package cache
...
//
// The cache is designed to efficiently handle billions of entries with
...
// size (maxBytes -> maxBucketBytes) in order to:
...
func calcMaxSlabChunks(maxBucketBytes uint64, maxChunks uint64) uint64 {
	...
	// - but at least 32KB to avoid too many tiny mmaps when filling quickly. for example when maxMB is 1 GB.
...
//
// The cache uses different configurations, allowing for flexibility in
// deployment across various environments with different memory requirements.
// See config.go for the active configuration.
// -------------------------------------------------------------------
...
type Stats struct {
	// TrimCount is the number of trim operations performed on the cache.
	TrimCount uint64
	// TotalMapSize is the total size of all hash maps used by the cache buckets.
	TotalMapSize uint64
	// TotalElementsAdded is the cumulative count of all elements ever added to the cache.
...
const (
	...
	// managing memory more aggressively.
	Trimmed
)
...
// storing key-value pairs with minimal GC pressure.
//
// Key features:
...
func New(maxBytes int, bucketType BucketType) (*ImprovedCache, error) {
	...
	case Unallocated:
		for i := 0; i < BucketsCount; i++ {
			...
			if err := c.buckets[i].Init(maxBucketBytes, 0); err != nil {
				return nil, fmt.Errorf("error creating unallocated cache: %w", err)
			}
		}
	case Preallocated:
	...
	default: // trimmed cache
		for i := 0; i < BucketsCount; i++ {
			...
			if err := c.buckets[i].Init(maxBucketBytes, 0); err != nil {
				return nil, fmt.Errorf("error creating trimmed cache: %w", err)
			}
		}
	}
...
//
// This method is optimized for scenarios where multiple keys need to be associated with
// the same value. It is particularly useful for batch processing.
//
// Parameters:
// - keys: Array of byte slices containing the keys
...
// SetMultiKeysSingleValueAppended stores multiple (k, v) entries in the cache, for the same v.
...
func (c *ImprovedCache) Get(dst *[]byte, k []byte) error {
	...
	if !c.buckets[idx].Get(dst, k, h, true) {
		return fmt.Errorf("key %v not found in cache: %w", k, ErrNotFound)
	}
...
// is no longer needed or when a complete refresh is required.
...
func (b *bucketTrimmed) Set(k, v []byte, h uint64, skipLocking ...bool) error {
	if len(k) >= (1<<maxValueSizeLog) || len(v) >= (1<<maxValueSizeLog) {
		...
		return fmt.Errorf("[bucketTrimmed.Set]too big key or value (key %d, value %d) max %d", len(k), len(v), 1<<maxValueSizeLog)
	}
	...
	if kvLen >= ChunkSize {
		...
		// fit a chunk.
		return fmt.Errorf("key, value, and k-v length %d bytes doesn't fit to a chunk %d bytes", kvLen, ChunkSize)
	}
...
// Appends v to the existing v value, doesn't overwrite.
func (b *bucketTrimmed) SetMultiKeysSingleValue(keys [][]byte, value []byte) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *bucketTrimmed) getChunk() ([]byte, error) {
	if len(b.freeChunks) == 0 {
		...
		if remaining == 0 {
			...
			return nil, fmt.Errorf("cannot allocate more chunks; bucket is at maxChunks=%d", maxChunks)
		}
		...
		if slabBytes <= 0 {
			return nil, fmt.Errorf("invalid slabBytes=%d (slabChunks=%d)", slabBytes, slabChunks)
		}
...
func (b *bucketPreallocated) SetMultiKeysSingleValue(keys [][]byte, value []byte) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *bucketPreallocated) Set(k, v []byte, h uint64, skipLocking ...bool) error {
	if len(k) >= (1<<maxValueSizeLog) || len(v) >= (1<<maxValueSizeLog) {
		// Too big key or value - its length cannot be encoded
		// with 2 bytes (see below). Skip the entry.
		return fmt.Errorf("[bucketPreallocated.Set]too big key or value (key %d, value %d) max %d", len(k), len(v), 1<<maxValueSizeLog)
	}
	...
	if len(chunk) == 0 {
		return fmt.Errorf("SHOULD NEVER ENTER HERE, chunk is nil or empty")
	}
...
// Get skips locking if skipLocking is set to true.
...
func (b *bucketUnallocated) Init(maxBytes uint64, _ int) error {
	...
	// calculate max slab chunks before beginning
	b.maxSlabChunks = calcMaxSlabChunks(maxBytes, maxChunks)
	if maxChunks > math.MaxInt {
		return fmt.Errorf("failed converting maxChunks: overflow")
	}
...
func (b *bucketUnallocated) Set(k, v []byte, h uint64, skipLocking ...bool) error {
	if len(k) >= (1<<maxValueSizeLog) || len(v) >= (1<<maxValueSizeLog) {
		// Too big key or value - its length cannot be encoded
		// with 2 bytes (see below). Skip the entry.
		return fmt.Errorf("[bucketUnallocated.Set] too big key or value (key %d, value %d) max %d", len(k), len(v), 1<<maxValueSizeLog)
	}
...
// SetMultiKeysSingleValue stores multiple (k, v) entries for the same bucket for a single v.
// Appends v to the existing v value, doesn't overwrite.
func (b *bucketUnallocated) SetMultiKeysSingleValue(keys [][]byte, value []byte) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *bucketUnallocated) getChunk() ([]byte, error) {
	if len(b.freeChunks) == 0 {
		...
		// we are at most creating remaining number of available chunks.
		if slabChunks > remaining {
			slabChunks = remaining
		}
