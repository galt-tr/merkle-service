# sql-store Specification

## Purpose

Defines the SQL-backed implementation of the merkle-service store interfaces. Covers supported drivers (PostgreSQL, SQLite), embedded schema migrations, transactional atomic semantics, the TTL sweeper that emulates Aerospike record expiry, connection pooling, and the PostgreSQL-backed end-to-end test that validates parity with the Aerospike backend.
